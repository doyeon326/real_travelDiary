...
<aside id="sidebar" class="nano">
  <div class="nano-content">
    ...
    <a class="compose-button" id="compose" href="${pageContext.request.contextPath}/viewMeetup">Compose</a>
...
<main id="main">
  ...
  <header class="header" style="padding-bottom:0px;">
    ...
    <h1 class="page-title"><a class="sidebar-toggle-btn trigger-toggle-sidebar"><span class="line"></span><span class="line"></span><span class="line"></span><span class="line line-angle1"></span><span class="line line-angle2"></span></a>Meetup<a><span class="icon glyphicon glyphicon-chevron-down"></span></a></h1>
 

  <div style="margin-top:70px; margin-bottom:0px;"><a href="${pageContext.request.contextPath}/meetupsortingfav">인기순</a> <a href="#">최신순</a> <a href="#">마감일순</a></div>
  </header>
...
<!--   <div class="action-bar">
  ...
  </div> header of the inbox ... icon and stuff-->
  <div id="main-nano-wrapper" class="nano">
    <div class="nano-content">
      <ul class="message-list">
      	<c:forEach var="listMeetup" items="${listMeetup}" varStatus="i">
   			
        <li class="green-dot unread" id="${i.index}">
          <div class="col col-1"><span class="dot"></span>
            <div class="checkbox-wrapper">
              <input type="checkbox" id="chk1">
              <label for="chk1" class="toggle"></label>
            </div>
            <div class="index${i.index}" style="display:none;"> ${listMeetup.getIdx()}</div>
            <p class="title">${listMeetup.getName()}</p><span class="star-toggle glyphicon glyphicon-star-empty"></span>
          </div>
          <div class="col col-2">
            <div class="subject">${listMeetup.getTitle()}&nbsp;&ndash;&nbsp; <span class="teaser">@LucasKriebel - Very cool :) Nicklas, You have a new direct message.</span></div>
            <div class="date">11:49 am</div> <!-- ${listMeetup.getDate()} -->
          </div>
        </li>
...
<div id="message">
  <div class="header">
    <h1 class="page-title" id="meetup_title"><a class="icon circle-icon glyphicon glyphicon-chevron-left trigger-message-close"></a>Food Testing in Seoul<span class="grey"></span></h1>
  <div class="info">   
     <p><a id="meetup_writer">글쓴이 / LV.55</a> <a id="LEVEL"></a>LV.55</p>
		<p id="board_idx" style="display:hidden"></p>
     
  <!--    <p>모집인원<a href="#" id="">You</a> to <a href="#">Scott Waite</a>, started on <a href="#">March 2, 2014</a> at 2:14 pm est.</p>
   -->   <p>어디서 <a href="#"id="meetup_destination">서울</a> , 모집기간 <a href="#" id="meetup_datestart">March 2, 2014</a> 부터 <a id="meetup_dateend"></a>까지</p>
      <p>모집인원 <a id="meetup_maxattendees"></a> 중 <a id="meetup_currentattendees"></a>명 모집완료</p>
     <div class="attendees" id="attendees" style="border: dashed 1px; display:inline-block; width:100%;">
     </div>
     
  		<div id="attendornot" class="attendYesOrNo" style="
  ...
  <div id="message-nano-wrapper" class="nano" style="top:550px;">
    <div class="nano-content">
      <ul class="message-container">
        <li class="sent">
          <div class="details">
            ...
            <div class="right" id="meetup_writedate">March 6, 2014, 20:08 pm</div>
          </div>
          
          
          <div class="messasge" id="meetup_contents">
           <!-- message -->
...
<script>
$(function(){
	$(".attendYesOrNo").click(function(){
	
		if($(this).text().trim() === "참석하기"){
			$(this).text("참석취소");
			
			var index = $("#board_idx").text().trim();
			$.ajax({
				type : "POST",
				url : "insertemeetupattend?meetupBoard_idx="+index,
				dataType: "json",
			...
			$.ajax({
				type : "POST",
				url : "deletemeetupattend?meetupBoard_idx="+index,
				dataType: "json",
				success : function(response){
					if(response==="success")
					alert("inserted");
...
$(function(){
	$(".unread").click(function(){
		//alert($(".index").text().trim());
		var index1 = $(this).attr('id'); //li's index
		//var num = $("div[class='index["+index+"]']");
	//	alert($(".index"+index1).text());
		var index = $(".index"+index1).text().trim();
		

		$.ajax({
			url:'viewMeetupDetails?meetupBoard_idx='+index,
			type:'POST',
			dataType:'json',

			success: function(response){ //성공했을경우 null 인지 아닌지 확인
			
					//$.each(response, function(idx,val){
					//	alert(response.title);
					$("#meetup_title").text(response.title);
					$("#meetup_writer").text(response.name);
					$("#meetup_destination").text(response.destination);
					$("#meetup_datestart").text(response.date_start);
					$("#meetup_dateend").text(response.date_end);
					$("#meetup_maxattendees").text(response.max_attendees);
					$("#meetup_currentattendees").text(response.attendees_count);
					$("#meetup_contents").text(response.contents);
					$("#meetup_writedate").text(response.date);
					$("#board_idx").text(response.idx);
					//$("#attendornot").text(response.attendStatus);

					
					if(response.attendStatus===1){
						$("#attendornot").text("참석취소");
						}
					else{
						$("#attendornot").text("참석하기");
						}


					var i;
					for(i=response.names.length; i>0; i--){
						$("#attendees").text(response.names);
...
$(function(){
	$("#compose").click(function(){
		window.location.href = "${pageContext.request.contextPath}/createMeetup";
		});
	});
